...
void EngineControlUnit::setEngineTemperature(float temp){
    ...
    if(temp > 120 || temp < -20){
        cout << "warning -> temperature must be within -20 to 120 celsius" << endl;
    }
}
...
void EngineControlUnit::DataLogHistory(){
...
void EngineControlUnit::setFuelInjectionRate(){
    ...
    //float current_temp = getEngineTemperature();
...
void EngineControlUnit::brakeControlInteraction(){
    static float brake_level = (float)(100 -  (float)((this->update_speed / 6000)*100));
