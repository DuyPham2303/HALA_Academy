...
// Thêm tính năng Log
/** 
 * @brief Khởi tạo ghi log với 1 cảm biến gốc
 * @param sensor -> con trỏe đến cảm biến cần thêm chức năng ghi log
 */
...
int main(int argc, char const *argv[])
{
    //đối tượng cảm biến gốcgốc
    Sensor* tempSensor = new TemperatureSensor();
 
    //thêm chức năng thông qua decorator
    Sensor* log = new LoggingSensor(tempSensor);
    log->readData();
