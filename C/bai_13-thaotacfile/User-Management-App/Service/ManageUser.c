...
Status Build_ListUser()
{
    ...
    UserData* pUserData = Read_UserDB();

    for(int i = 0 ; i < total_user ; i++)
    {
        Build_SelectSortType(&List_User_NameSorted,&pUserData[i],CompareName); 
    }
    for(int i = 0 ; i < total_user ; i++){
        Build_SelectSortType(&List_User_PhoneSorted,&pUserData[i],ComparePhone);
    }
